...
const executeSelectedAlgorithm = (selectedAlgorithm, num_of_processes, processes, setProcesses, setUpdatedProcesses, setExecuted, setStatisticsData, setAnotherUpdatedProcesses,setCurrentProcess) => {
    ...
    switch (selectedAlgorithm) {
        case FCFS:
            executeFCFS(processes, num_of_processes, setUpdatedProcesses,setCurrentProcess);
            setExecuted(true);
        ...
        case PREEMPTIVE_PRIORITY:
            executePreemptivePriority(processes, num_of_processes, setUpdatedProcesses, setAnotherUpdatedProcesses);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "PRE_PRIORITY", data }]);
            break;
        case STRF:
            executeSTRF(processes, num_of_processes, setUpdatedProcesses, setAnotherUpdatedProcesses);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "STRF", data }]);
            break;
        case ROUND_ROBIN:
            executeRoundRobin(processes, num_of_processes, setUpdatedProcesses, setAnotherUpdatedProcesses);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "ROUND_ROBIN", data }]);
            break;
        case MULTILEVEL_FEEDBACK_QUEUE:
            executeMultilevelFeedbackQueue(processes, num_of_processes, setUpdatedProcesses, setAnotherUpdatedProcesses);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
