...
const executeSelectedAlgorithm = (selectedAlgorithm, num_of_processes, processes, setProcesses, setExecuted, setStatisticsData,setCurrentProcess) => {
    ...
    switch (selectedAlgorithm) {
        case FCFS:
            executeFCFS(processes, num_of_processes, setProcesses,setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "FCFS", data }]);
            break;  
        case SJF:
            executeSJF(processes, num_of_processes, setProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "SJF", data }]);
            break;
        case PRIORITY:
            executePriority(processes, num_of_processes, setProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "PRIORITY", data }]);
            break;
        case PREEMPTIVE_PRIORITY:
            executePreemptivePriority(processes, num_of_processes, setProcesses, setAnotherUpdatedProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "PRE_PRIORITY", data }]);
            break;
        case STRF:
            executeSTRF(processes, num_of_processes, setProcesses, setAnotherUpdatedProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "STRF", data }]);
            break;
        case ROUND_ROBIN:
            executeRoundRobin(processes, num_of_processes, setProcesses, setAnotherUpdatedProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "ROUND_ROBIN", data }]);
            break;
        case MULTILEVEL_FEEDBACK_QUEUE:
            executeMultilevelFeedbackQueue(processes, num_of_processes, setProcesses, setAnotherUpdatedProcesses, setCurrentProcess);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
