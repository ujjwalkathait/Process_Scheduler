            ...
            <button
                ...
                onClick={e => {
                    ...
                    if (processes.length === 0) {
                        tost("Please add at least one process before proceeding.");
                        return;
                    }
                    if (arrivalTime === "" || burstTime === "" || priority === "") {
                        tost("Please fill in all fields before proceeding.");
                        return;
                    }
                    if (arrivalTime < 0 || burstTime < 0 || priority < 1) {
                        tost("Please enter valid values for Arrival Time, Burst Time, and Priority.");
                        return;
                    }
                    if (selectedAlgorithm === 3 || selectedAlgorithm === 4 || selectedAlgorithm === 7) {
                        if (priority === "") {
                            tost("Please enter a valid priority value.");
                            return;
                        }
                    }
                    if(selectedAlgorithm === 0){
                        tost("Please select a algorithm.");
                        return;
                    }
                    navigate('/simulate');
                }}
